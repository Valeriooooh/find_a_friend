...
use iroh::{base::node_addr::AddrInfoOptions, blobs::store::fs::Store, client::docs::{self, ShareMode}, docs::AuthorId, net::relay::RelayMode, node::{Node, RpcStatus}};
use common::Result;
use messages::basic::{DocumentCreateRequest, DocumentCreateResponse, DocumentListRequest, DocumentListResponse};
// use messages;
...
impl App {
    async fn new() -> Result<(Node<Store>, AuthorId)>{
        ...
        let node = node.enable_rpc_with_addr(iroh::node::DEFAULT_RPC_ADDR).await.unwrap();
...
async fn main() {
    ...
    rinf::debug_print!("startup succeded \n\n NodeId: {} \n\n AuthorId {}", node.node_id(), author.to_string());
...
async fn doc_list_get(node: Node<Store>) -> Result<()> {
    ...
    if let Ok(mut a) = node.docs().list().await{
        ...
        DocumentListResponse{document: docs.iter().map(|(x,_)| x.to_string()).collect()}.send_signal_to_dart();
    }
    Ok(())
}
...
async fn doc_create(node: Node<Store>, author: AuthorId) -> Result<()> {
    ...
    while let Some(dart_signal) = sig.recv().await {
        ...
        let _ = new_doc.set_bytes(author, "share_ticket", ticket.to_string()).await;
        DocumentCreateResponse{ticket: Some(ticket.to_string())}.send_signal_to_dart();
